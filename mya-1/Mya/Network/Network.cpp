...
std::vector<ClientMessage> Network::update() {
	...
	return std::vector<ClientMessage>();
}

void Network::lua_update(void* m)
{
	if (hasInit) {
		std::vector<ClientMessage> msgs;
		if (difftime(time(0), start) >= 1) {
		...
		if (msgs.size() > 0) {
			Mya* mya = (Mya*)m;
			sol::function event_keyDown = mya->lua->lua["event_networkMessage"];
			for (int i = 0; i < msgs.size(); i++) {
				if (event_keyDown != sol::nil)
					event_keyDown(msgs[i]);
				else
					std::cout << "event_networkMessage not registered\n";
			}
		}
	}
}
...
void Network::setPort(short a)
{
	PORT = a;
}

short Network::getPort()
{
	return PORT;
}
